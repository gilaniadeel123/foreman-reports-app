...
export default function Page() {
  ...
  const saveEntry = async () => {
    try {
      ...
      const entry_date = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(
        2,
        "0"
  ...
  const deleteEntryLocalListOnly = (id: string) => {
    // (Optional) This currently removes from the on-screen list only.
    // If you want to delete from Supabase too, we can add that later.
  ...
  const fillWeatherFromLocation = async () => {
    try {
      ...
      const weatherText = `${decodeWeatherCode(c.weather_code)} ${fmt(
        c.temperature_2m,
        "°C"
      )} | Wind ${fmt(c.wind_speed_10m, "km/h")} | RH ${fmt(
        c.relative_humidity_2m,
        "%"
      )} | Rain ${fmt(c.precipitation, "mm")}`.trim();
      ...
      setForm((s) => ({
        ...
        weather:
          s.weather ||
          `Weather lookup failed: ${err?.message || String(err)}`,
      }));
    }
  };
  ...
  const renderReportHTML = (e: Entry) => {
    ...
    const matTable =
      e.materialsRequired && e.materialItems.length
    ...
    return `
      <div style="font-family: ui-sans-serif, system-ui; padding: 24px; width: 794px;">
        ...
        <table style="width:100%; border-collapse: collapse; font-size: 12px; margin-bottom: 12px;">
          <tr>
            <td style="border:1px solid #999; padding:6px; width:50%"><strong>Project</strong><br/>${escapeHTML(
              FIXED_PROJECT
            )}</td>
            <td style="border:1px solid #999; padding:6px; width:50%"><strong>Area / Zone</strong><br/>${escapeHTML(
              e.area
            )}</td>
          </tr>
          <tr>
            <td style="border:1px solid #999; padding:6px"><strong>Weather</strong><br/>${escapeHTML(
              e.weather
            )}</td>
            <td style="border:1px solid #999; padding:6px"><strong>Manpower</strong><br/>${escapeHTML(
              e.manpower
            )}</td>
          </tr>
          <tr>
            <td style="border:1px solid #999; padding:6px"><strong>Obstacles</strong><br/>${
              escapeHTML(e.obstacles) || "-"
            }</td>
            <td style="border:1px solid #999; padding:6px"><strong>Safety Incidents</strong><br/>${
              escapeHTML(e.safetyIncidents) || "None"
            }</td>
          </tr>
          <tr>
            <td colspan="2" style="border:1px solid #999; padding:6px"><strong>Notes</strong><br/>${
              escapeHTML(e.notes) || "-"
            }</td>
          </tr>
        </table>
  ...
  return (
    <div className="container">
      ...
      <p className="muted">
        Prime 11 Unit 213 • Per-category progress, photos, auto-weather,
        obstacles, manpower, safety, and materials. Cloud-saved; export PDF/JSON.
      </p>
      ...
      <div className="card">
        ...
        <div className="card-body grid cols-1">
          <div className="grid cols-4">
            ...
            {/* Project is fixed; just show it */}
            <div>
              <label>Project</label>
              <input type="text" value={FIXED_PROJECT} readOnly />
            </div>
            ...
            <div>
              ...
              <div className="row">
                <input
                  ...
                  value={form.weather}
                  onChange={(e) =>
                    setForm((s) => ({ ...s, weather: e.target.value }))
                  }
                />
                <button
                  className="btn"
                  type="button"
                  onClick={fillWeatherFromLocation}
          ...
          <div
            className="card"
            style={{ border: "1px dashed var(--border)", marginTop: 8 }}
          >
                ...
                <AddCategory
                  onAdd={(name) => {
                    ...
                    setForm((s) => ({
                      ...s,
                      categoryProgress:
                        s.categoryProgress[name] !== undefined
                          ? s.categoryProgress
          ...
          <div className="grid cols-2">
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, obstacles: e.target.value }))
                }
              />
            </div>
            <div>
          ...
          <div className="grid cols-2">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, safetyIncidents: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="row" style={{ marginTop: 8 }}>
            <label style={{ margin: 0 }}>
              <input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, materialsRequired: e.target.checked }))
                }
              />{" "}
          ...
          {form.materialsRequired && (
            <div className="card" style={{ padding: 12, marginTop: 8 }}>
              <div className="row" style={{ justifyContent: "space-between" }}>
                ...
                <button className="btn" type="button" onClick={addMaterialItem}>
                  + Add Item
                </button>
              </div>
                  ...
                  <div key={m.id} className="grid cols-4">
                    <div>
                      ...
                      <input
                        ...
                        value={m.name}
                        onChange={(e) =>
                          updateMaterialItem(m.id, { name: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={m.quantity}
                        onChange={(e) =>
                          updateMaterialItem(m.id, { quantity: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={m.neededBy}
                        onChange={(e) =>
                          updateMaterialItem(m.id, { neededBy: e.target.value })
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        value={m.notes}
                        onChange={(e) =>
                          updateMaterialItem(m.id, { notes: e.target.value })
                        }
          ...
          <div className="card" style={{ padding: 12, marginTop: 8 }}>
            ...
            {form.photos.length === 0 ? (
              <div className="small" style={{ marginTop: 8 }}>
                Add progress photos (they will appear in the PDF).
              </div>
            ) : (
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="card-body">
          <strong>Today</strong>
          <div className="small">
            {todaysEntries.length} entr
            {todaysEntries.length === 1 ? "y" : "ies"} saved for{" "}
        ...
        {entries.map((e) => (
          <div key={e.id} className="card">
            <div className="card-body">
              <div className="row" style={{ justifyContent: "space-between" }}>
                ...
                <div className="row" style={{ gap: 6 }}>
                  ...
                  <span className="badge">
                    {summarizeProgress(e.categoryProgress)}
                  </span>
                </div>
              </div>
              ...
              <div className="grid cols-3" style={{ marginTop: 8 }}>
                <div className="small">
                  <strong>Weather:</strong> {e.weather || "-"}
                </div>
                <div className="small">
                  <strong>Manpower:</strong> {e.manpower || "-"}
                </div>
                <div className="small">
                  <strong>Safety:</strong> {e.safetyIncidents || "None"}
                </div>
              </div>
              <div className="small" style={{ marginTop: 6 }}>
                <strong>Obstacles:</strong> {e.obstacles || "-"}
              </div>
              <div className="small" style={{ marginTop: 6 }}>
            ...
            <div className="card-footer">
              ...
              <div className="row">
                <button className="btn" onClick={() => generatePDF(e)}>
                  PDF
                </button>
                <button
                  className="btn"
                  onClick={() => deleteEntryLocalListOnly(e.id)}
                >
                  Delete
                </button>
              </div>
